...
class DownloadDialog(wx.Dialog):
	def __init__(self, parent):
		...
		self.videoFormat = wx.Choice(panel, choices=["mp4", "mkv", "webm"])
		...
		self.audioFormat = wx.Choice(panel, choices=["mp3", "m4a", "opus", "flac", "wav"])
	...
	def buildCommand(self, url, outputDir):
		...
		if self.audioRadio.GetValue():
			...
			command.extend(["-x", "--audio-format", audioFormat])
